...
class MockBoto3Client:
    ...
    def put_item(self, TableName, Item, **kwargs):
        ...
        key = parsed_item.get("user_id") or parsed_item.get("session_id") or parsed_item.get("id")
        if not key:
        ...
        if "thalassemia" in prompt_lower or "hplc" in prompt_lower:
            if "risk" in prompt_lower or "genetic" in prompt_lower:
                response_text = json.dumps({
                    "risk_category": "HIGH",
                    "counseling_recommendations": "Both partners carry Beta Thalassemia Trait. Pre-marital genetic counseling and prenatal diagnosis are advised.",
            ...
            response_text = f"Thank you for contacting the Blood Warriors Awareness assistant. Thalassemia carrier screening involves a simple HPLC test. A carrier has a normal lifespan but may pass the trait."
